...
| 유동성 매칭 | 20점 | 거래 빈도와 상품 만기를 기준으로 고객의 자금 고정 가능성을 평가. 입출금 거래가 적은 고객은 중장기 상품에 높은 점수, 거래가 많은 고객은 짧은 만기·유동성 높은 상품에 높은 점수. 조기해지 허용 시 +2점 |
| 부가 혜택 | 10점 | 비과세 혜택 +6점, 중도해지 가능 +4점 |

#### 월 잉여자금 부족(monthlySavings ≤ 0) 예외 처리

월 잉여자금이 0 이하인 경우 신규 저축 여력이 부족한 것으로 판단한다. 적금 상품은 추천 후보에서 제외하고, 현재 보유 잔액을 기준으로 예금 중심 fallback 추천을 제공한다. 응답의 `fallbackReason` 필드에 사유 문구가 포함된다.
