...
`CASH_FLOW_RECOMMEND` 기능 실행 시 프런트엔드(`ChatbotWidget.tsx`)가 고객의 현금흐름 분석 결과와 모든 판매 중 상품을 받아 100점 만점으로 채점 후 상위 3개를 추천한다. 외부 AI 호출 없이 클라이언트 사이드에서 계산된다.

| 항목 | 배점 | 계산 기준 |
|---|---|---|
| 재정 적합도 | 40점 | 상품 최소 가입금액 ≤ 월 저축 가능액 (monthlySavings) 기준으로 선형 비례. 저축 성장형 진단 시 1.3× 가중치 |
| 예상 수익 | 30점 | bestRate 기준 12개월 기대 이자 / 500원 단위로 선형 스케일 (상한 30점) |
| 유동성 매칭 | 20점 | 선호 기간(periodPref)과 상품 기간 범위 일치도: 완전 포함 20점 / 부분 겹침 10점 / 불일치 2점 |
| 부가 혜택 | 10점 | 세제 혜택(+4), 조기해지(+3), 자동 갱신(+2), 복리(+1) 합산 |

**유동성 매칭 기준**

| 고객 진단 | 선호 기간 (periodPref) |
|---|---|
| 단기 유동성 필요 (`lowBalance`, `highExpense`) | 1~12개월 |
| 저축 성장형 (`growingSavings`) | 24~60개월 |
| 기본 | 12~36개월 |

**연령 기반 상품 필터링 (3단계)**

1. **DB 기반 (1순위)**: `GET /products` 응답의 `targetGroups[].minAge/maxAge`와 고객 만 나이 비교 — 연령 제한 있는 상품은 범위 밖이면 제외
2. **키워드 폴백 (2순위)**: target groups 정보가 없는 상품은 상품명에 `청년도약·청년우대·청년주택` 등 포함 시 만 19~34세만 통과
3. **필터 생략 (3순위)**: 고객 생년월일 조회 실패 시 연령 필터 없이 전체 점수 기준으로만 추천

> 고객 나이는 `GET /api/v1/customers/me` 응답의 `birthDate`(YYYYMMDD 또는 YYYY-MM-DD)를 파싱해 만 나이로 계산한다.

**fallback 추천 (monthlySavings ≤ 0)**

진단 결과 월 잉여금이 0 이하인 경우 점수 기반 추천 대신 최저 가입금액 기준 상위 3개를 fallback으로 추천한다.
