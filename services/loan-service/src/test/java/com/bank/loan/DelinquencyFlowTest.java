...
class DelinquencyFlowTest extends AbstractLoanIntegrationTest {
    ...
    @Test @Order(10)
    void baseDate_가_due와_같으면_overdue_없음() throws Exception {
        mockMvc.perform(post("/api/internal/delinquency/rollover").param("baseDate", DUE_1))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.newlyOverdueInstallments").value(0))
                .andExpect(jsonPath("$.data.activeDelinquencies").value(0));
    }
    ...
    void 회차1_overdue_dlq_신규생성() throws Exception {
        mockMvc.perform(post("/api/internal/delinquency/rollover").param("baseDate", "20300202"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.newlyOverdueInstallments").value(1))
                .andExpect(jsonPath("$.data.activeDelinquencies").value(1))
                .andExpect(jsonPath("$.data.snapshotsCreated").value(1));
    ...
    void 동일_baseDate_재실행_멱등() throws Exception {
        mockMvc.perform(post("/api/internal/delinquency/rollover").param("baseDate", "20300202"))
                ...
                .andExpect(jsonPath("$.data.newlyOverdueInstallments").value(0))
                .andExpect(jsonPath("$.data.activeDelinquencies").value(1))
                .andExpect(jsonPath("$.data.snapshotsCreated").value(0));
    ...
    void 회차_상환후_해소() throws Exception {
        ...
        mockMvc.perform(post("/api/internal/delinquency/rollover").param("baseDate", "20300303"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.resolvedDelinquencies").value(1));
    }
