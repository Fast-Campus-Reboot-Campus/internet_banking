 ...
 *   계약 체결 시 maturity 자동 생성 (original=current=20610101)
 *
 * 시나리오:
 *   10) 자동 생성 확인 — status ACTIVE, original=current=20610101, count=0
 *   11) 6개월 연장 → current=20610701, count=1, last_extended_date=today
 *   12) 다시 12개월 연장 → current=20620701, count=2
...
class MaturityFlowTest extends AbstractLoanIntegrationTest {
    ...
    @Test @Order(10)
    void 약정체결시_자동생성() throws Exception {
        mockMvc.perform(get("/api/loan-contracts/{cntrId}/maturity", cntrId))
                ...
                .andExpect(jsonPath("$.data.currentMaturityDate").value(EXPECTED_ORIGINAL_MATURITY))
    ...
    void 육개월_연장() throws Exception {
        mockMvc.perform(post("/api/loan-contracts/{cntrId}/maturity/extend", cntrId)
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.originalMaturityDate").value(EXPECTED_ORIGINAL_MATURITY))
                .andExpect(jsonPath("$.data.currentMaturityDate").value("20610701"))
    ...
    void 다시_십이개월_연장_누적() throws Exception {
        mockMvc.perform(post("/api/loan-contracts/{cntrId}/maturity/extend", cntrId)
                        ...
                        .content("""
                                ...
                                """))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.currentMaturityDate").value("20620701"))
