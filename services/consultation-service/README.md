...
상품 추천 시 우대금리가 있는 상품은 **가입 조건을 함께 안내**합니다.

### 동작 방식

1. `_execute_cash_flow_recommend()` 실행 시 추천 후보 상품의 우대금리 조건(`condition_description`)을 `banking_deposit_product_interest_rates` 테이블에서 조회
2. 각 상품 dict에 `pref_condition` 필드로 추가
3. 아래 두 경로 모두에 조건 반영:

| 경로 | 반영 방식 |
|------|----------|
| **LLM 추천** (`recommend()`) | 상품 목록 텍스트에 `[우대조건: ...]` 포함 → LLM이 답변에 자연어로 안내 |
| **룰 기반 추천** (`_make_reason()`) | 추천 이유 문자열에 `(우대조건: ...)` 포함 |

### 출력 예시

```
기본 3.0% + 우대 0.5% = 최대 3.5% (우대조건: 자동이체 등록 시 / 당행 급여이체 등록 시)
· 월 500,000원 납입(잉여자금의 20%), 납입 후 2,000,000원 여유 → 12개월 이자 약 11,375원 예상
```

> 우대금리 조건이 없는 상품(조건 없이 기본금리만 있는 경우)은 조건 안내 없이 금리만 표시됩니다.
