...
```
1. 고객 계좌 전체 잔액 합산 → total_balance
...
4. 고객 유형 진단 (순서대로 적용)
     잉여자금 ≤ 0                  → 지출 과다형 (예금만)
     잔액 ≥ 잉여자금 × 12          → 목돈 보유형 (예금 우선)
     잔액 < 잉여자금 × 12           → 저축 성장형 (적금 우선)
5. 가입 불가 상품 제외
     예금: 잔액 < 최소가입금액 → 제외
     적금: 잉여자금 ≤ 0 또는 잉여자금 < 최소납입액 × 2 → 제외
6. 개인고객(target_group_id=1) 상품만 대상 — 군인·청년 전용 제외
7. 100점 체계로 점수 산정 (아래 참조)
8. 상위 3개 상품을 1~3위로 반환
```
...
| 항목 | 배점 | 산식 |
|---|---|---|
| 재정 적합도 | 40점 | 예금: `min(잔액/최소가입금액, 5)/5 × 40` |
| | | 적금: `min(잉여자금/(최소납입액×2), 5)/5 × 40` |
| 예상 수익 | 30점 | 전체 후보 중 최고 수익 대비 정규화 × 30 |
| | | 예금: `최소가입금액 × 금리/100 × (최소기간/12)` |
| | | 적금: `최소납입액 × 최소기간 × 금리/100 / 2` |
| 유동성 매칭 | 20점 | 월 거래 10건↑: 단기 상품 우선 / 5건↓: 장기 허용 |
| 혜택 | 10점 | 세금혜택 +7, 중도해지 가능 +3 |

### 잉여자금 산정 기준

- `direction_type = 'IN'` → 입금 (급여, 이자, 적금 납입 수령 등)
- `direction_type = 'OUT'` → 출금 (이체, 출금, 대출 상환, 적금 납입 등)
- 기간: **최근 3개월** 거래만 집계
- 거래 유형(transaction_type)이 아닌 방향(direction_type) 기준을 사용하여 LOAN_PAYMENT 등 신규 거래 유형을 자동 처리
...
최근 3개월 거래 내역 최대 20건을 반환합니다. `customer_no` 미제공 시 `AUTH_REQUIRED` 응답.
