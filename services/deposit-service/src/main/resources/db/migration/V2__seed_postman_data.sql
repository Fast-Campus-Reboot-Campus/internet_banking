...
INSERT INTO departments (department_id, department_code, department_name, department_type, is_active)
VALUES
    (1, 'DEP-PRODUCT', '수신상품부', 'PRODUCT', TRUE),
    (2, 'DEP-SALES', '수신영업부', 'SALES', TRUE)
ON CONFLICT (department_id) DO NOTHING;

INSERT INTO branches (branch_id, branch_code, branch_name, department_id, address, phone_number, region, is_active, opened_at)
VALUES
    (1, 'BR-001', '테스트본점', 2, '서울특별시 중구 테스트로 1', '02-0000-0001', 'SEOUL', TRUE, '20260101')
ON CONFLICT (branch_id) DO NOTHING;

INSERT INTO employees (employee_id, employee_number, employee_name, department_id, branch_id, position, role, employment_status, phone_number, email, joined_at)
VALUES
    (1, 'EMP-001', '테스트매니저', 2, 1, 'Manager', 'MANAGER', 'ACTIVE', '010-0000-0001', 'manager@example.com', '20260101')
ON CONFLICT (employee_id) DO NOTHING;

INSERT INTO products (
    product_id, product_type, product_name, description, department_id, base_interest_rate,
...
INSERT INTO deposit_products (deposit_product_id, product_id, deposit_type, is_compound_interest)
VALUES
    (1, 1, 'TERM', FALSE)
ON CONFLICT (deposit_product_id) DO NOTHING;

INSERT INTO savings_products (savings_product_id, product_id, saving_type, monthly_payment_min_amount, monthly_payment_max_amount)
VALUES
    (1, 2, 'FREE', 10000.00, 1000000.00)
ON CONFLICT (savings_product_id) DO NOTHING;

INSERT INTO subscription_products (product_id, monthly_payment_amount, min_monthly_payment, max_monthly_payment, max_recognized_payment_amount)
VALUES
    (3, 100000.00, 20000.00, 500000.00, 10000000.00)
ON CONFLICT (product_id) DO NOTHING;

INSERT INTO target_groups (target_group_id, target_group_name, description, is_active)
VALUES
...
VALUES
    ...
    (2, 2),
    (3, 1)
ON CONFLICT (product_id, target_group_id) DO NOTHING;

INSERT INTO product_join_channels (product_join_channel_id, product_id, join_channel_code)
VALUES
    (1, 1, 'WEB'),
    (2, 1, 'MOBILE'),
    (3, 2, 'WEB'),
    (4, 2, 'MOBILE'),
    (5, 3, 'BRANCH')
ON CONFLICT (product_join_channel_id) DO NOTHING;

INSERT INTO product_interest_rates (
    rate_id, product_id, rate_type, minimum_contract_period, maximum_contract_period,
...
INSERT INTO product_special_terms (product_special_term_id, product_id, special_term_id, is_required)
VALUES
    (1, 1, 1, TRUE),
    (2, 2, 2, FALSE)
ON CONFLICT (product_special_term_id) DO NOTHING;

INSERT INTO contracts (
    contract_id, contract_number, customer_id, product_id, join_amount,
    ...
    contract_status, join_channel, branch_id, manager_id
)
VALUES
    (1, 'CTR-20260519-0001', 'CUST001', 1, 1000000.00, 3.20, 0.30, 3.50, 'GENERAL', 12, '20260519', '20270519', TRUE, FALSE, NULL, 'ACTIVE', 'WEB', NULL, 1),
    (2, 'CTR-20260519-0002', 'CUST001', 2, 50000.00, 3.60, 0.00, 3.60, 'GENERAL', 24, '20260519', '20280519', FALSE, TRUE, 25, 'ACTIVE', 'MOBILE', NULL, 1),
    (3, 'CTR-20260519-0003', 'CUST_NO_ACCOUNT', 1, 300000.00, 3.20, 0.00, 3.20, 'GENERAL', 12, '20260519', '20270519', FALSE, FALSE, NULL, 'ACTIVE', 'WEB', NULL, 1)
...
INSERT INTO contract_applied_rates (applied_rate_id, contract_id, rate_id, applied_rate, condition_verified_yn)
VALUES
    (1, 1, 1, 3.20, TRUE),
    (2, 1, 2, 0.30, TRUE),
    (3, 2, 3, 3.60, TRUE)
...
INSERT INTO contract_special_term_agreements (
...
SELECT setval(pg_get_serial_sequence('departments', 'department_id'), COALESCE((SELECT MAX(department_id) FROM departments), 1), TRUE);
SELECT setval(pg_get_serial_sequence('branches', 'branch_id'), COALESCE((SELECT MAX(branch_id) FROM branches), 1), TRUE);
...
SELECT setval(pg_get_serial_sequence('deposit_products', 'deposit_product_id'), COALESCE((SELECT MAX(deposit_product_id) FROM deposit_products), 1), TRUE);
SELECT setval(pg_get_serial_sequence('savings_products', 'savings_product_id'), COALESCE((SELECT MAX(savings_product_id) FROM savings_products), 1), TRUE);
SELECT setval(pg_get_serial_sequence('product_join_channels', 'product_join_channel_id'), COALESCE((SELECT MAX(product_join_channel_id) FROM product_join_channels), 1), TRUE);
SELECT setval(pg_get_serial_sequence('target_groups', 'target_group_id'), COALESCE((SELECT MAX(target_group_id) FROM target_groups), 1), TRUE);
SELECT setval(pg_get_serial_sequence('product_interest_rates', 'rate_id'), COALESCE((SELECT MAX(rate_id) FROM product_interest_rates), 1), TRUE);
SELECT setval(pg_get_serial_sequence('special_terms', 'special_term_id'), COALESCE((SELECT MAX(special_term_id) FROM special_terms), 1), TRUE);
SELECT setval(pg_get_serial_sequence('product_special_terms', 'product_special_term_id'), COALESCE((SELECT MAX(product_special_term_id) FROM product_special_terms), 1), TRUE);
SELECT setval(pg_get_serial_sequence('contracts', 'contract_id'), COALESCE((SELECT MAX(contract_id) FROM contracts), 1), TRUE);
SELECT setval(pg_get_serial_sequence('accounts', 'account_id'), COALESCE((SELECT MAX(account_id) FROM accounts), 1), TRUE);
SELECT setval(pg_get_serial_sequence('contract_applied_rates', 'applied_rate_id'), COALESCE((SELECT MAX(applied_rate_id) FROM contract_applied_rates), 1), TRUE);
SELECT setval(pg_get_serial_sequence('contract_special_term_agreements', 'special_agreement_id'), COALESCE((SELECT MAX(special_agreement_id) FROM contract_special_term_agreements), 1), TRUE);
SELECT setval(pg_get_serial_sequence('interest_history', 'interest_id'), COALESCE((SELECT MAX(interest_id) FROM interest_history), 1), TRUE);
