...
type TerminateStep = 'method' | 'done' | 'error'
type TerminateState = {
  step: TerminateStep
  accountId: number
...
export default function ChatbotWidget() {
  ...
  const RATE_GUIDE_KEYWORDS = ['금리', '이자율', '이자 얼마', '금리 알려', '금리 보여', '모든 금리', '전체 금리']
  ...
  async function handleQuickAction(action: (typeof quickActions)[number]) {
    ...
    if (action.type === 'cashflow') {
      await handleFeature('MY_CASH_FLOW', action.message, false)
      return
    }
  ...
  async function startTerminate(row: Record<string, unknown>) {
    ...
    try {
      ...
      checkingAccounts = accs.filter(a => a.type === '입출금').map(a => ({
        account_id: a.apiAccountId ?? 0,
        account_number: a.number,
        balance: a.balance,
                        ...
                        <div className="mt-3 space-y-2">
                          {productRows.map((row, index) => (
                            <div key={`${message.id}-${index}`} className="rounded border border-kb-border bg-white p-3 text-xs space-y-1">
                              <div className="flex items-center gap-2">
                                {row.rank && <span className="flex-shrink-0 rounded-full bg-[#2D6A4F] text-white text-[10px] font-bold w-5 h-5 flex items-center justify-center">{String(row.rank)}</span>}
                              ...
                              <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-[11px] text-kb-text-muted">
                                ...
                                <span>기간 {row.min_period_month != null ? `${row.min_period_month}${row.max_period_month && row.max_period_month !== row.min_period_month ? `~${row.max_period_month}` : ''}개월` : '-'}</span>
                                {row.target_groups && <span>가입대상 {String(row.target_groups)}</span>}
                                {row.description && <span>{String(row.description)}</span>}
                              </div>
                              {row.reason && <p className="text-[11px] text-[#2D6A4F] font-medium">✓ {String(row.reason)}</p>}
                              {(row.min_join_amount || row.max_join_amount) && (
                                <p className="text-[11px] text-kb-text-muted">
                                  가입금액 {row.min_join_amount ? `${Number(row.min_join_amount).toLocaleString()}원~` : ''}{row.max_join_amount ? `${Number(row.max_join_amount).toLocaleString()}원` : ''}
                                </p>
                              )}
                            </div>
                          ))}
                          {totalPages > 1 && (
                            <div className="flex justify-between text-[11px] text-kb-text-muted">
                              <button type="button" disabled={page === 0} onClick={() => setDataPage(message.id, page - 1)} className="disabled:opacity-30">{TEXT.prev}</button>
                              <span>{page + 1} / {totalPages}</span>
                              <button type="button" disabled={page >= totalPages - 1} onClick={() => setDataPage(message.id, page + 1)} className="disabled:opacity-30">{TEXT.next}</button>
                            </div>
                          )}
                        </div>
                      )
